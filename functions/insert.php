...
if(isset($_POST['save'])){
    ...
    $query = "select branch2, start_date, end_date from travel_info where branch2 = '$branch2' and start_date ='$start_date' and end_date = '$end_date' and user = '$username'";
    ...
    if($row['branch2']==$branch2 && $row['start_date'] == $start_date && $row['end_date']== $end_date){
        ...
        if($result){
             ...
             echo '<script>
                    alert("Success | Your data has been inserted");
                    window.location = "../TF/details.php";
                </script>';
